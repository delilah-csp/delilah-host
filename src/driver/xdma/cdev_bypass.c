...
static int
copy_desc_data(struct xdma_transfer* transfer, char __user* buf,
               size_t* buf_offset, size_t buf_size)
{
  ...
  for (i = 0; i < transfer->desc_num; i++) {
    if (*buf_offset + sizeof(struct xdma_desc) <= buf_size) {
      copy_err = copy_to_user(&buf[*buf_offset], transfer->desc_virt + i,
...
static ssize_t
char_bypass_write(struct file* file, const char __user* buf, size_t count,
                  loff_t* pos)
{
  struct xdma_dev* xdev;
  struct xdma_engine* engine;
  ...
  bypass_addr =
    (void __iomem*)((u32 __iomem*)bypass_addr + engine->bypass_offset);
