...
inline void
__write_register(const char* fn, u32 value, void __iomem* base,
                 unsigned int off)
{
...
inline u32
__read_register(const char* fn, void __iomem* base, unsigned int off)
{
...
static int
xvc_shift_bits(void __iomem* base, u32 tms_bits, u32 tdi_bits, u32* tdo_bits)
{
  u32 control;
  int count;
...
static long
xvc_ioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
  ...
  rv =
    copy_from_user((void*)&xvc_obj, (void __user*)arg, sizeof(struct xvc_ioc));
  /* anything not copied ? */
  if (rv) {
  ...
  rv = copy_from_user((void*)tms_buf, (const char __user*)xvc_obj.tms_buf,
                      total_bytes);
  if (rv) {
    pr_info("copy tmfs_buf failed: %d/%u.\n", rv, total_bytes);
    goto cleanup;
  }
  rv = copy_from_user((void*)tdi_buf, (const char __user*)xvc_obj.tdi_buf,
                      total_bytes);
  if (rv) {
  ...
  for (bits = 0, bits_left = total_bits; bits < total_bits;
       bits += 32, bits_left -= 32) {
...
void
cdev_xvc_init(struct xdma_cdev* xcdev)
{
