...
static int
xdma_thread_cmpl_status_pend(struct list_head* work_item)
{
  struct xdma_engine* engine =
    list_entry(work_item, struct xdma_engine, cmplthp_list);
  int pend = 0;
  unsigned long flags;
...
static int
xdma_thread_cmpl_status_proc(struct list_head* work_item)
{
  struct xdma_engine* engine;
...
static inline int
xthread_work_pending(struct xdma_kthread* thp)
{
...
static inline void
xthread_reschedule(struct xdma_kthread* thp)
{
  if (thp->timeout) {
    pr_debug_thread("%s rescheduling for %u seconds", thp->name, thp->timeout);
...
int
xdma_kthread_start(struct xdma_kthread* thp, char* name, int id)
{
  int len;
  int node;
  ...
  thp->task =
    kthread_create_on_node(xthread_main, (void*)thp, node, "%s", thp->name);
  if (IS_ERR(thp->task)) {
    pr_err("kthread %s, create task failed: 0x%lx\n", thp->name,
           (unsigned long)IS_ERR(thp->task));
    thp->task = NULL;
    return -EFAULT;
  }
...
int
xdma_kthread_stop(struct xdma_kthread* thp)
{
  int rv;
...
void
xdma_thread_remove_work(struct xdma_engine* engine)
{
...
void
xdma_thread_add_work(struct xdma_engine* engine)
{
...
int
xdma_threads_create(unsigned int num_threads)
{
  ...
  cs_threads = kzalloc(num_threads * sizeof(struct xdma_kthread), GFP_KERNEL);
  if (!cs_threads) {
