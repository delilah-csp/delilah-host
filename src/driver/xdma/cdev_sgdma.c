...
MODULE_PARM_DESC(h2c_timeout,
                 "H2C sgdma timeout in seconds, default is 10 sec.");

unsigned int c2h_timeout = 10;
module_param(c2h_timeout, uint, 0644);
MODULE_PARM_DESC(c2h_timeout,
                 "C2H sgdma timeout in seconds, default is 10 sec.");
...
async_io_handler(unsigned long cb_hndl, int err)
{
  ...
  if (!err)
    numbytes = xdma_xfer_completion(
      (void*)cb, xdev, engine->channel, cb->write, cb->ep_addr, &cb->sgt, 0,
...
static int
check_transfer_align(struct xdma_engine* engine, const char __user* buf,
                     size_t count, loff_t pos, int sync)
{
  if (!engine) {
...
static ssize_t
char_sgdma_read_write(struct file* file, const char __user* buf, size_t count,
                      loff_t* pos, bool write)
{
  ...
  dbg_tfr("file 0x%p, priv 0x%p, buf 0x%p,%llu, pos %llu, W %d, %s.\n", file,
          file->private_data, buf, (u64)count, (u64)*pos, write, engine->name);
...
static ssize_t
char_sgdma_write(struct file* file, const char __user* buf, size_t count,
                 loff_t* pos)
{
...
static ssize_t
cdev_aio_write(struct kiocb* iocb, const struct iovec* io, unsigned long count,
               loff_t pos)
{
  ...
  for (i = 0; i < count; i++) {
    ...
    rv = xdma_xfer_submit_nowait((void*)&caio->cb[i], xdev, engine->channel,
                                 caio->cb[i].write, caio->cb[i].ep_addr,
                                 &caio->cb[i].sgt, 0, h2c_timeout * 1000);
  }
...
static ssize_t
cdev_aio_read(struct kiocb* iocb, const struct iovec* io, unsigned long count,
              loff_t pos)
{
  ...
  for (i = 0; i < count; i++) {
    ...
    rv = xdma_xfer_submit_nowait((void*)&caio->cb[i], xdev, engine->channel,
                                 caio->cb[i].write, caio->cb[i].ep_addr,
                                 &caio->cb[i].sgt, 0, c2h_timeout * 1000);
  }
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0)
static ssize_t
cdev_write_iter(struct kiocb* iocb, struct iov_iter* io)
{
  return cdev_aio_write(iocb, io->iov, io->nr_segs, io->iov_offset);
}

static ssize_t
cdev_read_iter(struct kiocb* iocb, struct iov_iter* io)
{
...
static int
ioctl_do_aperture_dma(struct xdma_engine* engine, unsigned long arg, bool write)
{
  ...
  dbg_tfr("%s, W %d, buf 0x%lx,%lu, ep %llu, aperture %u.\n", engine->name,
          write, io.buffer, io.len, io.ep_addr, io.aperture);
  ...
  rv = check_transfer_align(engine, (char*)io.buffer, io.len, io.ep_addr, 1);
  if (rv) {
    pr_info("Invalid transfer alignment detected\n");
    return rv;
  }
  ...
  io.error = 0;
  res = xdma_xfer_aperture(engine, write, io.ep_addr, io.aperture, &cb.sgt, 0,
                           write ? h2c_timeout * 1000 : c2h_timeout * 1000);
