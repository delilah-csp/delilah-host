...
static const char* const devnode_names[] = {
  ...
  XDMA_NODE_NAME "%d_xvc",           XDMA_NODE_NAME "%d_events_%d",
  XDMA_NODE_NAME "%d_h2c_%d",        XDMA_NODE_NAME "%d_c2h_%d",
  XDMA_NODE_NAME "%d_bypass_h2c_%d", XDMA_NODE_NAME "%d_bypass_c2h_%d",
...
ssize_t
xdma_dev_instance_show(struct device* dev, struct device_attribute* attr,
                       char* buf)
{
...
config_kobject(struct xdma_cdev* xcdev, enum cdev_type type)
{
  ...
  switch (type) {
    ...
    case CHAR_BYPASS_C2H:
      ...
      rv = kobject_set_name(&xcdev->cdev.kobj, devnode_names[type], xdev->idx,
                            engine->channel);
      break;
    case CHAR_BYPASS:
    case CHAR_USER:
...
create_sys_device(struct xdma_cdev* xcdev, enum cdev_type type)
{
  ...
  xcdev->sys_device =
    device_create(g_xdma_class, &xdev->pdev->dev, xcdev->cdevno, NULL,
...
static int
create_xcdev(struct xdma_pci_dev* xpdev, struct xdma_cdev* xcdev, int bar,
             struct xdma_engine* engine, enum cdev_type type)
{
  int rv;
  int minor;
  ...
  if (!xpdev->major) {
    ...
    int rv = alloc_chrdev_region(&dev, XDMA_MINOR_BASE, XDMA_MINOR_COUNT,
                                 XDMA_NODE_NAME);
...
xpdev_create_interfaces(struct xdma_pci_dev* xpdev)
{
  ...
  rv = create_xcdev(xpdev, &xpdev->ctrl_cdev, xdev->config_bar_idx, NULL,
                    CHAR_CTRL);
  if (rv < 0) {
  ...
  if (xdev->bypass_bar_idx > 0) {
    for (i = 0; i < xpdev->h2c_channel_max; i++) {
      ...
      rv = create_xcdev(xpdev, &xpdev->bypass_h2c_cdev[i], i, engine,
                        CHAR_BYPASS_H2C);
      if (rv < 0) {
    ...
    for (i = 0; i < xpdev->c2h_channel_max; i++) {
      ...
      rv = create_xcdev(xpdev, &xpdev->bypass_c2h_cdev[i], i, engine,
                        CHAR_BYPASS_C2H);
      if (rv < 0) {
    ...
    rv = create_xcdev(xpdev, &xpdev->bypass_cdev_base, xdev->bypass_bar_idx,
                      NULL, CHAR_BYPASS);
    if (rv < 0) {
  ...
  if (xdev->user_bar_idx >= 0) {
    rv = create_xcdev(xpdev, &xpdev->user_cdev, xdev->user_bar_idx, NULL,
                      CHAR_USER);
    if (rv < 0) {
    ...
    /* xvc */
    rv =
      create_xcdev(xpdev, &xpdev->xvc_cdev, xdev->user_bar_idx, NULL, CHAR_XVC);
    if (rv < 0) {
...
xdma_cdev_init(void)
{
  ...
  cdev_cache = kmem_cache_create("cdev_cache", sizeof(struct cdev_async_io), 0,
