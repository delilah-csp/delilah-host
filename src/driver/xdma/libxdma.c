...
MODULE_PARM_DESC(enable_st_c2h_credit,
                 "Set 1 to enable ST C2H engine credit feature, default is 0 ( "
...
#define xlx_wait_event_interruptible swait_event_interruptible_exclusive
#elif HAS_SWAKE_UP
#define xlx_wake_up swake_up
#define xlx_wait_event_interruptible_timeout swait_event_interruptible_timeout
#define xlx_wait_event_interruptible swait_event_interruptible
#else
...
#define xlx_wait_event_interruptible wait_event_interruptible
#endif
...
xdev_list_add(struct xdma_dev* xdev)
{
  ...
  if (list_empty(&xdev_list)) {
    ...
    if (poll_mode) {
      int rv =
        xdma_threads_create(xdev->h2c_channel_max + xdev->c2h_channel_max);
      if (rv < 0) {
 ...
 */
static u32
xdma_get_next_adj(unsigned int remaining, u32 next_lo)
{
   ...
   */
  next_index =
    ((next_lo & (XDMA_PAGE_SIZE - 1)) >> 5) % XDMA_MAX_ADJ_BLOCK_SIZE;
...
static int
engine_err_handle(struct xdma_engine* engine, struct xdma_transfer* transfer,
                  u32 desc_completed)
{
  u32 value;
  int rv = 0;
    ...
    if (engine->eop_flush) {
      ...
      for (i = 0; i < max; i++) {
        if ((result[i].status & RX_STATUS_EOP) != 0) {
          transfer->flags |= XFER_FLAG_ST_C2H_EOP_RCVED;
          break;
        }
      }
      ...
      pr_info("%s, xfer 0x%p, stopped half-way, %d/%d.\n", engine->name,
              transfer, *pdesc_completed, transfer->desc_num);
...
engine_service(struct xdma_engine* engine, int desc_writeback)
{
  ...
  dbg_tfr("%s wb 0x%x, desc_count %u, err %u, dequeued %u.\n", engine->name,
          desc_writeback, desc_count, err_flag, engine->desc_dequeued);
...
static int
identify_bars(struct xdma_dev* xdev, int* bar_id_list, int num_bars,
              int config_bar_pos)
{
  /*
  ...
  pr_info("%d BARs: config %d, user %d, bypass %d.\n", num_bars, config_bar_pos,
          xdev->user_bar_idx, xdev->bypass_bar_idx);
  return 0;
}
#endif
...
enable_msi_msix(struct xdma_dev* xdev, struct pci_dev* pdev)
{
  ...
  if ((interrupt_mode == 3 || !interrupt_mode) &&
...
dump_desc(struct xdma_desc* desc_virt)
{
  ...
  for (j = 0; j < 8; j += 1) {
    pr_info("0x%08lx/0x%02lx: 0x%08x 0x%08x %s\n", (uintptr_t)p,
            (uintptr_t)p & 15, (int)*p, le32_to_cpu(*p), field_name[j]);
    p++;
  }
  pr_info("\n");
}
...
transfer_dump(struct xdma_transfer* transfer)
{
  ...
  pr_info("xfer 0x%p, state 0x%x, f 0x%x, dir %d, len %u, last %d.\n", transfer,
          transfer->state, transfer->flags, transfer->dir, transfer->len,
          transfer->last_in_request);

  pr_info("transfer 0x%p, desc %d, bus 0x%llx, adj %d.\n", transfer,
          transfer->desc_num, (u64)transfer->desc_bus, transfer->desc_adjacent);
 ...
 */
static void
xdma_desc_link(struct xdma_desc* first, struct xdma_desc* second,
 ...
 */
static inline void
xdma_desc_done(struct xdma_desc* desc_virt, int count)
{
 ...
 */
static void
xdma_desc_set(struct xdma_desc* desc, dma_addr_t rc_bus_addr, u64 ep_addr,
              int len, int dir)
{
  /* transfer length */
...
transfer_abort(struct xdma_engine* engine, struct xdma_transfer* transfer)
{
  ...
  pr_info("abort transfer 0x%p, desc %d, engine desc queued %d.\n", transfer,
          transfer->desc_num, engine->desc_dequeued);
...
engine_free_resource(struct xdma_engine* engine)
{
  ...
  if (engine->desc) {
    ...
    dma_free_coherent(&xdev->pdev->dev,
                      engine->desc_max * sizeof(struct xdma_desc), engine->desc,
                      engine->desc_bus);
    engine->desc = NULL;
  }
  ...
  if (!list_empty(&engine->transfer_list)) {
    ...
    if (transfer->cyclic) {
      ...
      if (engine->perf_buf_virt)
        dma_free_coherent(&engine->xdev->pdev->dev, size, engine->perf_buf_virt,
                          engine->perf_buf_bus);
...
engine_alloc_resource(struct xdma_engine* engine)
{
  ...
  if (poll_mode) {
    engine->poll_mode_addr_virt =
      dma_alloc_coherent(&xdev->pdev->dev, sizeof(struct xdma_poll_wb),
  ...
  if (engine->streaming && engine->dir == DMA_FROM_DEVICE) {
    engine->cyclic_result = dma_alloc_coherent(
      &xdev->pdev->dev, engine->desc_max * sizeof(struct xdma_result),
...
static int
engine_init(struct xdma_engine* engine, struct xdma_dev* xdev, int offset,
            enum dma_data_direction dir, int channel)
{
  int rv;
  u32 val;
...
static int
transfer_build(struct xdma_engine* engine, struct xdma_request_cb* req,
               struct xdma_transfer* xfer, unsigned int desc_max)
{
  ...
  for (; i < desc_max; i++, j++, sdesc++) {
    dbg_desc("sw desc %d/%u: 0x%llx, 0x%x, ep 0x%llx.\n", i + req->sw_desc_idx,
             req->sw_desc_cnt, sdesc->addr, sdesc->len, req->ep_addr);

    /* fill in descriptor entry j with transfer details */
    xdma_desc_set(xfer->desc_virt + j, sdesc->addr, req->ep_addr, sdesc->len,
                  xfer->dir);
    xfer->len += sdesc->len;
              ...
              struct xdma_transfer* xfer)
{
  unsigned int desc_max =
    min_t(unsigned int, req->sw_desc_cnt - req->sw_desc_idx, engine->desc_max);
  int i = 0;
  int last = 0;
  u32 control;
  unsigned long flags;
  ...
  xfer->desc_bus =
    engine->desc_bus + (sizeof(struct xdma_desc) * engine->desc_idx);
  xfer->res_bus =
    engine->cyclic_result_bus + (sizeof(struct xdma_result) * engine->desc_idx);
  ...
  if (engine->eop_flush) {
    for (i = 0; i < last; i++)
      xdma_desc_control_set(xfer->desc_virt + i, XDMA_DESC_COMPLETED);
    xfer->desc_cmpl_th = 1;
  } else
  ...
  for (i = 0; i < xfer->desc_num; i++) {
    u32 next_adj =
      xdma_get_next_adj(xfer->desc_num - i - 1, (xfer->desc_virt + i)->next_lo);
...
xdma_request_cb_dump(struct xdma_request_cb* req)
{
  ...
  pr_info("request 0x%p, total %u, ep 0x%llx, sw_desc %u, sgt 0x%p.\n", req,
          req->total_len, req->ep_addr, req->sw_desc_cnt, req->sgt);
  sgt_dump(req->sgt);
...
ssize_t
xdma_xfer_aperture(struct xdma_engine* engine, bool write, u64 ep_addr,
                   unsigned int aperture, struct sg_table* sgt, bool dma_mapped,
                   int timeout_ms)
{
  struct xdma_dev* xdev;
  ...
  if (!dma_mapped) {
    sgt->nents = pci_map_sg(xdev->pdev, sgt->sgl, sgt->orig_nents, dir);
    if (!sgt->nents) {
  ...
  while (req->offset < req->total_len) {
    ...
    xfer->desc_bus = desc_bus =
      engine->desc_bus + (sizeof(struct xdma_desc) * desc_idx);
    xfer->res_bus =
      engine->cyclic_result_bus + (sizeof(struct xdma_result) * desc_idx);
    ...
    for (sg = req->sg; i < sg_max && desc_idx < desc_max;
      ...
      while (tlen) {
        unsigned int len = min_t(unsigned int, tlen, maxlen);
        ...
        xdma_desc_set(engine->desc + desc_idx, addr, ep_addr, len, dir);
    ...
    desc_virt->control = cpu_to_le32(DESC_MAGIC | XDMA_DESC_STOPPED |
    ...
    for (i = 0; i < desc_cnt; i++) {
      u32 next_adj =
        xdma_get_next_adj(desc_cnt - i - 1, (xfer->desc_virt + i)->next_lo);
    ...
    dbg_tfr("xfer %u,%u/%u, ep 0x%llx/0x%x, done %ld, sg %u/%u, desc %u.\n",
            xfer->len, req->offset, req->total_len, req->ep_addr, req->aperture,
            done, req->sg_idx, sg_max, desc_cnt);
    ...
    if (timeout_ms > 0)
      xlx_wait_event_interruptible_timeout(
        xfer->wq, (xfer->state != TRANSFER_STATE_SUBMITTED),
...
ssize_t
xdma_xfer_submit(void* dev_hndl, int channel, bool write, u64 ep_addr,
...
ssize_t
xdma_xfer_completion(void* cb_hndl, void* dev_hndl, int channel, bool write,
...
ssize_t
xdma_xfer_submit_nowait(void* cb_hndl, void* dev_hndl, int channel, bool write,
                        u64 ep_addr, struct sg_table* sgt, bool dma_mapped,
                        int timeout_ms)
{
  struct xdma_dev* xdev = (struct xdma_dev*)dev_hndl;
  ...
  while (nents) {
    ...
    dbg_tfr("xfer %p, len %u, ep 0x%llx, sg %u/%u. nents = %d\n", xfer,
            xfer->len, req->ep_addr, req->sw_desc_idx, req->sw_desc_cnt, nents);
...
xdma_performance_submit(struct xdma_dev* xdev, struct xdma_engine* engine)
{
  ...
  if (!engine->desc) {
    engine->desc = dma_alloc_coherent(
      &xdev->pdev->dev, num_desc_in_a_loop * sizeof(struct xdma_desc),
      &engine->desc_bus, GFP_KERNEL);
    if (!engine->desc) {
...
static int
probe_for_engine(struct xdma_dev* xdev, enum dma_data_direction dir,
                 int channel)
{
  ...
  if ((engine_id != engine_id_expected) || (channel_id != channel)) {
    dbg_init("%s %d engine, reg off 0x%x, id mismatch 0x%x,0x%x,exp 0x%x,0x%x, "
             "SKIP.\n",
             dir == DMA_TO_DEVICE ? "H2C" : "C2H", channel, offset, engine_id,
             channel_id, engine_id_expected, channel_id != channel);
    return -EINVAL;
  }
...
void*
xdma_device_open(const char* mname, struct pci_dev* pdev, int* user_max,
...
int
xdma_user_isr_register(void* dev_hndl, unsigned int mask, irq_handler_t handler,
                       void* dev)
{
  struct xdma_dev* xdev = (struct xdma_dev*)dev_hndl;
  int i;
