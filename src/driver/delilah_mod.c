	...
	case DELILAH_SUCCESS:
		...
		if (ebpf_ret) {
			dev_warn(&delilah->dev,
				"Delilah returned with status 0x%x but eBPF return 0x%llx (expected 0)\n",
...
static void hpdev_free(struct delilah_pci_dev *hpdev)
{
...
static struct delilah_pci_dev *hpdev_alloc(struct pci_dev *pdev)
{
...
static int probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	hndl = xdma_device_open(DRV_MODULE_NAME, pdev, &max_user_irqs, &hpdev->h2c_channel_max,
...
static inline struct xdma_channel *xdma_get_chnl(struct xdma_channel *channels, __u8 slot, __u8 max)
{
	return &channels[slot % max];
}

struct xdma_channel *xdma_get_c2h(struct delilah_pci_dev *hpdev, __u8 slot)
{
	return xdma_get_chnl(hpdev->xdma_c2h_chnl, slot, hpdev->c2h_channel_max);
}

struct xdma_channel *xdma_get_h2c(struct delilah_pci_dev *hpdev, __u8 slot)
{
	return xdma_get_chnl(hpdev->xdma_h2c_chnl, slot, hpdev->h2c_channel_max);
}
...
static int __init delilah_mod_init(void)
{
	int rv;
	pr_info("%s", version);
...
static void __exit delilah_mod_exit(void)
{
